    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-8 card">
          ...
          <div class="card-body">
            <form id="model-<?=$modelname?>" action="<?=$href_base . $model_id . '/save'?>" method="POST">
            <?php foreach ($model as $key => $value) {
              $type = (strlen($value) > 32) ? 'textarea' : 'text';
              switch ($type) {
                case 'textarea':
                  $form_input = '<textarea class="form-control" id="model-'.$key.'" name="'.$modelname.'['.$key.']" rows="3">'.$value.'</textarea>';
                  $form_label = '<label class="form-label" for="model-'.$key.'">'.$key.'</label>';
                  break;
                
                default:
                  $form_input = '<input class="form-control" id="model-'.$key.'" name="'.$modelname.'['.$key.']" type="text" name="model'.$key.'" value="'.$value.'" onchange="changed(this);">';
                  $form_label = '<label class="form-label" for="model-'.$key.'">'.$key.'</label>';
                  break;
              }
              ?>
        ...
        <div class="col-4 card">
          ...
          <div class="card-body">
            ...
            <a href="<?=$href_base . 'list'?>" class="btn btn-lg btn-block btn-outline-danger">Cancel</a>
          </div>
        </div>
      </div>
    </div>
